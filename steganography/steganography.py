...
class Steganography:
    ...
    def encode(source, message, bit_split):
        ...
        header = [bit_split_str, padding, message_length, message_type, num_extras]
        ...
        if header.shape[0] + message.message.shape[0] > source.source.shape[0]:
            e = (
                ...
                str(header.shape[0] + message.message.shape[0]),
                ">",
                str(source.source.shape[0]),
            )
            raise Exception(e)
        ...
        encoded[
            offset : offset + message.message.shape[0], -bit_split:
        ] = message.message
